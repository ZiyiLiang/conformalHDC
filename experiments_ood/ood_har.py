...
def run_single_experiment(random_state):
    ...
    # Train/Cal/Test Split on ID data (Match 80% Train, 15% Calib, 5% Test)
    X_train_cal, X_test, y_train_cal, y_test = train_test_split(
        X_id, y_id, test_size=0.05, random_state=random_state, stratify=y_id
    )
    X_train, X_cal, y_train, y_cal = train_test_split(
        X_train_cal, y_train_cal, test_size=(0.15/0.95), random_state=random_state, stratify=y_train_cal
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python ood_har.py <seed_group_id>")
        sys.exit(1)
