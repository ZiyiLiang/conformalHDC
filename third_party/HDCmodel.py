...
# This file is adapted from the third party repo: https://github.com/Raisony/HD_CARDIO

# Loading training and testing data
def dataLoader(base_path, dataset):
    file_path = os.path.join(base_path, dataset + '.pkl')
...
def HV_encoding(HDC, baseVector, levelVector, Data):
    HV = []
...
def main(dimension, iteration, training, testing):
    ...
    HVector = HV_encoding(HDC, baseVector, levelVector, trainingData)
    HVector_test = HV_encoding(HDC, baseVector, levelVector, testingData)
...
class HyperDimensionalComputing(object):
    ...
    def retraining(self, classHVs, trainHVs, trainLabels, testHVs, testLabels, n_iteration):
        ...
        for i in range(n_iteration):
            ...
            print('Epoch', i+1, 'accuracy:', currAcc)
            if currAcc > bestacc:
                bestAcc = currAcc
    ...
    def predict(self, classHVs, testHVs, testLabels):
        preds = []
        for index in range(len(testHVs)):
            predict, _ = checkVector(classHVs, testHVs[index])
